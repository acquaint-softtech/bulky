...
/** @typedef {import('@bulky/core').bulky} bulky */
/** @typedef {import('@bulky/core').bulkyFile} bulkyFile */
/** @typedef {import('@bulky/core').FailedbulkyFile<{}>} FailedbulkyFile */
...
export default class Tus extends BasePlugin {
  ...
  handleResetProgress() {
    const files = { ...this.bulky.getState().files };
   ...
   *
   * @param {bulkyFile} file for use with upload
  ...
  upload(file) {
    ...
    return new Promise((resolve, reject) => {
      ...
      // We override tus fingerprint to bulky’s `file.id`, since the `file.id`
      ...
      uploadOptions.onSuccess = () => {
        ...
        if (upload.url) {
          this.bulky.log(`Download ${upload.file.name} from ${upload.url}`);
        }
  ...
  onReceiveUploadUrl(file, uploadURL) {
    const currentFile = this.bulky.getFile(file.id);
    if (!currentFile) return;
    // Only do the update if we didn't have an upload URL yet.
    if (!currentFile.tus || currentFile.tus.uploadUrl !== uploadURL) {
      this.bulky.log("[Tus] Storing upload url");
      this.bulky.setFileState(currentFile.id, {
  ...
  onPause(fileID, cb) {
    this.uploaderEvents[fileID].on("upload-pause", (targetFileID, isPaused) => {
      if (fileID === targetFileID) {
        // const isPaused = this.bulky.pauseResume(fileID)
        cb(isPaused);
      }
    });
  }
  ...
  /**
   * @param {(bulkyFile | FailedbulkyFile)[]} files
   */
  uploadFiles(files) {
  ...
  handleUpload(fileIDs) {
    if (fileIDs.length === 0) {
      this.bulky.log("[Tus] No files to upload");
      return Promise.resolve();
    }

    if (this.opts.limit === 0) {
      this.bulky.log(
        "[Tus] When uploading multiple files at once, consider setting the `limit` option (to `10` for example), to limit the number of concurrent uploads, which helps prevent memory and network issues: https://bulky.io/docs/tus/#limit-0",
        "warning"
      );
    }

    this.bulky.log("[Tus] Uploading...");
    const filesToUpload = fileIDs.map((fileID) => this.bulky.getFile(fileID));
  ...
  install() {
    this.bulky.setState({
      capabilities: {
        ...this.bulky.getState().capabilities,
        resumableUploads: true,
      },
    });
    this.bulky.addUploader(this.handleUpload);

    this.bulky.on("reset-progress", this.handleResetProgress);
  }

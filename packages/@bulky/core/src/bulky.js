...
class bulky {
   ...
   * Instantiate bulky
   *
   * @param {object} opts — bulky options
   */
  constructor(opts) {
    ...
    // Exposing bulky object on window for debugging and testing
  ...
  setFileState(fileID, state) {
    ...
    this.setState({
      files: {
        ...this.getState().files,
        [fileID]: { ...this.getState().files[fileID], ...state },
      },
    });
  }
   ...
   * @param { bulkyFile | undefined } file
   */
  ...
  #checkAndCreateFileStateObject(files, fileDescriptor) {
    // bulky expects files in { name, type, size, data } format.
    // If the actual File object is passed from input[type=file] or drag-drop,
    // we normalize it to match bulky file object
    ...
    if (this.checkIfFileAlreadyExists(fileID)) {
      const error = new RestrictionError(
        this.i18n("noDuplicates", { fileName })
      );
  ...
  addFiles(fileDescriptors) {
    ...
    for (let i = 0; i < fileDescriptors.length; i++) {
      try {
        let newFile = this.#checkAndCreateFileStateObject(
          files,
          fileDescriptors[i]
        );
        // Users are asked to re-select recovered files without data,
  ...
  use(Plugin, opts) {
    ...
    if (existsPluginAlready) {
      const msg =
        ...
        "bulky plugins must have unique `id` options. See https://bulky.io/docs/plugins/#id.";
      throw new Error(msg);
    }
  ...
  [Symbol.for("bulky test: getPlugins")](type) {
  ...
  /**
   * Uninstall all plugins and close down this bulky instance.
   */
  close({ reason } = {}) {
    this.log(
      `Closing bulky instance ${this.opts.id}: removing all files and uninstalling plugins`
    );
   ...
   * If `opts.logger: bulky.debugLogger` or `opts.debug: true`, logs to the browser console.
   *
  ...
  #createUpload(fileIDs, opts = {}) {
    // bulky.retryAll sets this to true — when retrying we want to ignore `allowNewUpload: false`
  ...
  [Symbol.for("bulky test: createUpload")](...args) {
  ...
  async #runUpload(uploadID) {
    ...
    if (currentUpload) {
      ...
      // introduced code in the @bulky/core upload-success handler to prepare postprocessing progress

...
const sampleImage = fs.readFileSync(
  new URL("../../../../e2e/cypress/fixtures/images/image.jpg", import.meta.url)
);
...
describe("src/Core", () => {
  ...
  it("should expose a class", () => {
    const core = new Core();
    expect(core.constructor.name).toEqual("bulky");
  });

  it('should have a string `id` option that defaults to "bulky"', () => {
    const core = new Core();
    expect(core.getID()).toEqual("bulky");
  ...
  describe("plugins", () => {
    it("should add a plugin to the plugin stack", () => {
      ...
      expect(
        Object.keys(core[Symbol.for("bulky test: getPlugins")]("acquirer"))
          .length
      ).toEqual(1);
    });
    ...
    it("should call the specified method on all the plugins", () => {
      ...
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[0].mocks.run.mock
          .calls.length
      ).toEqual(1);
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[0].mocks.run.mock
      ...
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[1].mocks.run.mock
          .calls.length
      ).toEqual(1);
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[1].mocks.run.mock
  ...
  describe("state", () => {
    it("should update all the plugins with the new state when the updateAll method is called", () => {
      ...
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[0].mocks.update
          .mock.calls.length
      ).toEqual(1);
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[0].mocks.update
      ...
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[1].mocks.update
          .mock.calls.length
      ).toEqual(1);
      expect(
        core[Symbol.for("bulky test: getPlugins")]("acquirer")[1].mocks.update
  ...
  it("should clear all uploads and files on cancelAll()", () => {
    ...
    const id = core[Symbol.for("bulky test: createUpload")](fileIDs);
  ...
  it("should close, reset and uninstall when the close method is called", () => {
    ...
    const coreCancelEventMock = jest.fn();
    const coreStateUpdateEventMock = jest.fn();
    const plugin = core[Symbol.for("bulky test: getPlugins")]("acquirer")[0];
  ...
  describe("postprocessors", () => {
    ...
    it("should execute all the postprocessors when uploading a file", () => {
      ...
      return core.upload().then(() => {
        ...
        const fileId = "bulky-foo/jpg-1e-image";
  ...
  describe("adding a file", () => {
    ...
    it("does not dedupe different files", async () => {
      ...
      expect(
        core.getFile("bulky-foo/jpg-1e-image/jpeg-17175-1562770350937")
      ).toBeDefined();
      expect(
        core.getFile("bulky-foo//jpg-1l3o-1e-image/jpeg-17175-1562770350937")
      ).toBeDefined();
    });
  });
  ...
  describe("uploading a file", () => {
    ...
    it("should only upload files that are not already assigned to another upload id", () => {
      ...
      core.store.state.currentUploads = {
        upload1: {
          fileIDs: [
            "bulky-file1/jpg-1e-image/jpeg",
            "bulky-file2/jpg-1e-image/jpeg",
            "bulky-file3/jpg-1e-image/jpeg",
          ],
        },
        upload2: {
          fileIDs: [
            "bulky-file4/jpg-1e-image/jpeg",
            "bulky-file5/jpg-1e-image/jpeg",
            "bulky-file6/jpg-1e-image/jpeg",
          ],
        },
      };
      core.addUploader(() => Promise.resolve());
      ...
      return expect(
        core.upload().then(validateNanoID)
  ...
  describe("setOptions", () => {
    it("should change options on the fly", () => {
      ...
      expect(core.opts.id).toEqual("lolbulky");
  ...
  describe("createUpload", () => {
    it("should assign the specified files to a new upload", () => {
      ...
      core[Symbol.for("bulky test: createUpload")](
  ...
  describe("log", () => {
    ...
    it("should log to console when logger: bulky.debugLogger or debug: true is set", () => {

...
import { Socket, Provider, RequestClient } from "@bulky/companion-client";
...
import { createAbortError } from "@bulky/utils/lib/AbortController";
...
class HTTPCommunicationQueue {
  ...
  async uploadFile(file, chunks, signal) {
    ...
    throwIfAborted(signal);
    return this.#sendCompletionRequest(file, {
      key,
      uploadId,
      parts,
      signal,
  ...
  async resumeUploadFile(file, chunks, signal) {
    ...
    const { uploadId, key } = await this.getUploadId(file, signal);
    throwIfAborted(signal);
    const alreadyUploadedParts = await this.#listParts(file, {
      uploadId,
      key,
      signal,
    }).abortOn(signal);
    ...
    for (;;) {
      ...
      try {
        return {
          ...
          ...(await this.#uploadPartBytes(signature, body, signal).abortOn(
            signal
          )),
        };
      } catch (err) {
...
export default class AwsS3Multipart extends BasePlugin {
  ...
  constructor(bulky, opts) {
    ...
    this.#client = new RequestClient(bulky, opts);
  ...
  [Symbol.for("bulky test: getClient")]() {
    return this.#client;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.addEventListener("load", (ev) => {
        ...
        if (etag === null) {
          reject(
            new Error(
              "AwsS3/Multipart: Could not read the ETag header. This likely means CORS is not configured correctly on the S3 Bucket. See https://bulky.io/docs/aws-s3-multipart#S3-Bucket-Configuration for instructions."
            )
          );
          return;
        }
  ...
  #setS3MultipartState = (file, { key, uploadId }) => {
    const cFile = this.bulky.getFile(file.id);
  ...
  uploadFile(file) {
    return new Promise((resolve, reject) => {
      ...
      const onSuccess = (result) => {
        ...
        const cFile = this.bulky.getFile(file.id);
        this.bulky.emit("upload-success", cFile || file, uploadResp);

        if (result.location) {
          this.bulky.log(`Download ${file.name} from ${result.location}`);
        }
      ...
      const onPartComplete = (part) => {
        ...
        this.bulky.emit("s3-multipart:part-uploaded", cFile, part);
      };
  ...
  async connectToServerSocket(file) {
    return new Promise((resolve, reject) => {
      ...
      socket.on("error", (errData) => {
        this.bulky.emit("upload-error", file, new Error(errData.error));
        this.resetUploaderReferences(file.id);
        queuedRequest.done();
  ...
  async upload(fileIDs) {
    ...
    const promises = fileIDs.map((id) => {
      const file = this.bulky.getFile(id);
      if (file.isRemote) {
  ...
  install() {
    ...
    this.bulky.addPreProcessor(this.#setCompanionHeaders);
    this.bulky.addUploader(this.upload);
  }
  ...
  uninstall() {
    ...
    this.bulky.removePreProcessor(this.#setCompanionHeaders);
    this.bulky.removeUploader(this.upload);
  }
}

...
describe("AwsS3Multipart", () => {
  ...
  it("Registers AwsS3Multipart upload plugin", () => {
    ...
    const pluginNames = core[Symbol.for("bulky test: getPlugins")](
      "uploader"
    ...
    it("Keeps chunks marked as busy through retries until they complete", async () => {
      ...
      awsS3Multipart.setOptions({
        ...
        createMultipartUpload: jest.fn((file) => {
          ...
          let busy = false;
          let done = false;
          busySpy = jest.fn((value) => {
            busy = value;
          });
          doneSpy = jest.fn((value) => {
            done = value;
          });
  ...
  describe("MultipartUploader", () => {
    ...
    const signPart = jest.fn(async (file, { partNumber }) => {
      return {
  ...
  describe("dynamic companionHeader", () => {
    ...
    it("companionHeader is updated before uploading file", async () => {
      ...
      const client = awsS3Multipart[Symbol.for("bulky test: getClient")]();

      expect(
        client[Symbol.for("bulky test: getCompanionHeaders")]().authorization
      ).toEqual(newToken);
    });
  });
});

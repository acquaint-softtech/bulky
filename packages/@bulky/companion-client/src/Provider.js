...
export default class Provider extends RequestClient {
  ...
  async headers() {
    ...
    const authHeaders = {};
    if (token) {
      authHeaders["bulky-auth-token"] = token;
    }

    if (this.companionKeysParams) {
      authHeaders["bulky-credentials-params"] = btoa(
  ...
  onReceiveResponse(response) {
    ...
    const plugin = this.bulky.getPlugin(this.pluginId);
  ...
  getAuthToken() {
    return this.bulky.getPlugin(this.pluginId).storage.getItem(this.tokenKey);
  }
  ...
  authUrl(queries = {}) {
    ...
    if (this.preAuthToken) {
      params.set("bulkyPreAuthToken", this.preAuthToken);
    }
  ...
  logout() {
    return this.get(`${this.id}/logout`)
      .then((response) =>
        Promise.all([
          response,
          this.bulky.getPlugin(this.pluginId).storage.removeItem(this.tokenKey),
        ])
      )
  ...
  static initPlugin(plugin, opts, defaultOpts) {
    ...
    if (opts.serverUrl || opts.serverPattern) {
      throw new Error(
        "`serverUrl` and `serverPattern` have been renamed to `companionUrl` and `companionAllowedHosts` respectively in the 0.30.5 release. Please consult the docs (for example, https://bulky.io/docs/instagram/ for the Instagram plugin) and use the updated options.`"
      );
    }

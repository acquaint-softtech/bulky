...
import fetchWithNetworkError from "@bulky/utils/lib/fetchWithNetworkError";
...
export default class RequestClient {
  ...
  [Symbol.for("bulky test: getCompanionHeaders")]() {
    return this.#companionHeaders;
  }

  get hostname() {
    const { companion } = this.bulky.getState();
  ...
  async headers() {
    const defaultHeaders = {
      ...
      "bulky-Versions": `@bulky/companion-client=${RequestClient.VERSION}`,
    };
  ...
  onReceiveResponse({ headers }) {
    const state = this.bulky.getState();

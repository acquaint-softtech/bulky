...
import dataURItoBlob from "@bulky/utils/lib/dataURItoBlob";
import isObjectURL from "@bulky/utils/lib/isObjectURL";
...
export default class ThumbnailGenerator extends UIPlugin {
  ...
  /**
   * Create a thumbnail for the given bulky file object.
   *
  ...
  setPreviewURL(fileID, preview) {
    this.bulky.setFileState(fileID, { preview });
  }
  ...
  processQueue() {
    ...
    if (this.queue.length > 0) {
      const current = this.bulky.getFile(this.queue.shift());
      if (!current) {
        this.bulky.log(
    ...
    this.queueProcessing = false;
    this.bulky.log("[ThumbnailGenerator] Emptied thumbnail queue");
    this.bulky.emit("thumbnail:all-generated");
    return Promise.resolve();
  }
    ...
    if (isPreviewSupported(file.type) && !file.isRemote) {
      return this.createThumbnail(
        ...
        .catch((err) => {
          ...
          this.bulky.emit("thumbnail:error", this.bulky.getFile(file.id), err);
        });
    }
    return Promise.resolve();
  }
  ...
  onRestored = () => {
    const restoredFiles = this.bulky
      .getFiles()
  ...
  waitUntilAllProcessed = (fileIDs) => {
    ...
    return new Promise((resolve) => {
      if (this.queueProcessing) {
        this.bulky.once("thumbnail:all-generated", () => {
  ...
  install() {
    this.bulky.on("file-removed", this.onFileRemoved);
    this.bulky.on("cancel-all", this.onAllFilesRemoved);

    if (this.opts.lazy) {
      this.bulky.on("thumbnail:request", this.onFileAdded);
      this.bulky.on("thumbnail:cancel", this.onCancelRequest);
    } else {
      this.bulky.on("file-added", this.onFileAdded);
      this.bulky.on("restored", this.onRestored);
    }

    if (this.opts.waitForThumbnailsBeforeUpload) {
      this.bulky.addPreProcessor(this.waitUntilAllProcessed);
    }
  }

  uninstall() {
    this.bulky.off("file-removed", this.onFileRemoved);
    this.bulky.off("cancel-all", this.onAllFilesRemoved);

    if (this.opts.lazy) {
      this.bulky.off("thumbnail:request", this.onFileAdded);
      this.bulky.off("thumbnail:cancel", this.onCancelRequest);
    } else {
      this.bulky.off("file-added", this.onFileAdded);
      this.bulky.off("restored", this.onRestored);
    }

    if (this.opts.waitForThumbnailsBeforeUpload) {
      this.bulky.removePreProcessor(this.waitUntilAllProcessed);
    }
  }
}

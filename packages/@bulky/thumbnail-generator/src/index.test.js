...
describe("uploader/ThumbnailGeneratorPlugin", () => {
  ...
  describe("install", () => {
    it("should subscribe to bulky file-added event", () => {
  ...
  describe("uninstall", () => {
    it("should unsubscribe from bulky file-added event", () => {
      const core = Object.assign(new MockCore(), {
        on: jest.fn(),
        off: jest.fn(),
      });
    ...
    it("should revoke object URLs when files are removed", async () => {
      ...
      try {
        ...
        plugin.createThumbnail = jest.fn(async () => {
          await delay(50);
          return "blob:http://bulky.io/fake-thumbnail";
        });
  ...
  describe("events", () => {
    const core = new MockCore();
    const plugin = new ThumbnailGeneratorPlugin(core);
    plugin.createThumbnail = jest.fn((file) =>

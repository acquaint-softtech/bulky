...
import getFileTypeExtension from "@bulky/utils/lib/getFileTypeExtension";
import mimeTypes from "@bulky/utils/lib/mimeTypes";
import isMobile from "is-mobile";
import canvasToBlob from "@bulky/utils/lib/canvasToBlob";
...
export default class Webcam extends UIPlugin {
    ...
    this.hasCameraCheck().then((hasCamera) => {
      ...
      return this.mediaDevices
        ...
        .catch((err) => {
          ...
          this.bulky.info(err.message, "error");
        });
    });
  }
  ...
  submit() {
    try {
      if (this.capturedMediaFile) {
        this.bulky.addFile(this.capturedMediaFile);
      }
    } catch (err) {
      // Logging the error, exept restrictions, which is handled in Core
      if (!err.isRestriction) {
        this.bulky.log(err, "error");
      }
    }
  }
  ...
  getVideoElement() {
    return this.el.querySelector(".bulky-Webcam-video");
  }
  ...
  oneTwoThreeSmile() {
    return new Promise((resolve, reject) => {
      ...
      const countDown = setInterval(() => {
        ...
        if (count > 0) {
          this.bulky.info(`${count}...`, "warning", 800);
          count--;
        } else {
          clearInterval(countDown);
          this.bulky.info(this.i18n("smile"), "success", 1500);
  ...
  takeSnapshot() {
    ...
    this.opts
      ...
      .catch((err) => {
        ...
        this.bulky.info(message, "error", 5000);

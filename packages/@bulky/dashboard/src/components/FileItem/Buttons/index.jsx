...
function EditButton({
  ...
  if (
    ...
    return (
      <button
        className="bulky-u-reset bulky-c-btn bulky-Dashboard-Item-action bulky-Dashboard-Item-action--edit"
        type="button"
...
function RemoveButton({ i18n, onClick, file }) {
  return (
    <button
      className="bulky-u-reset bulky-Dashboard-Item-action bulky-Dashboard-Item-action--remove"
      type="button"
...
const copyLinkToClipboard = (event, props) => {
  copyToClipboard(
    ...
    .then(() => {
      props.bulky.log("Link copied to clipboard.");
      props.bulky.info(props.i18n("copyLinkToClipboardSuccess"), "info", 3000);
    })
    .catch(props.bulky.log)
    // avoid losing focus
...
function CopyLinkButton(props) {
  ...
  return (
    <button
      className="bulky-u-reset bulky-Dashboard-Item-action bulky-Dashboard-Item-action--copyLink"
      type="button"
...
export default function Buttons(props) {
  ...
  return (
    <div className="bulky-Dashboard-Item-actionWrapper">
      <EditButton
        i18n={i18n}
        file={file}
      ...
      {showLinkToFileUploadResult && file.uploadURL ? (
        <CopyLinkButton file={file} bulky={bulky} i18n={i18n} />
      ) : null}
      {showRemoveButton ? (
        <RemoveButton
          i18n={i18n}
          file={file}
          bulky={bulky}
          onClick={() => props.bulky.removeFile(file.id, "removed-by-user")}
        />
      ) : null}
    </div>
  );

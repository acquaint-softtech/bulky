...
describe("ReduxStore", () => {
  ...
  it("fires `subscribe` if state is modified externally (eg redux devtools)", () => {
    const combinedReducers = Redux.combineReducers({ bulky: reducer });
    const r = Redux.createStore((state, action) => {
      // Add a `SET` action that can change bulky state without going through the bulky reducer or action creator.
    ...
    r.dispatch({
      ...
      payload: {
        bulky: {
          [store[Symbol.for("bulky test: get id")]()]: { b: 2 },
        },
      },
    });

...
export default class ProviderView extends View {
  ...
  logout() {
    this.provider
      ...
      .then((res) => {
        if (res.ok) {
          if (!res.revoked) {
            const message = this.plugin.bulky.i18n("companionUnauthorizeHint", {
              provider: this.plugin.title,
              url: res.manual_revoke_url,
            });
            this.plugin.bulky.info(message, "info", 7000);
          }
  ...
  addFolder(folder) {
    ...
    return this.listAllFiles(folder.requestPath)
      .then((files) => {
        let count = 0;
        ...
        files.forEach((file) => {
          ...
          if (!this.plugin.bulky.checkIfFileAlreadyExists(id)) {
            count++;
          }
        });
        ...
        if (count === 0) {
          message = this.plugin.bulky.i18n("folderAlreadyAdded", {
            folder: folder.name,
          });
        } else if (files.length) {
          message = this.plugin.bulky.i18n("folderAdded", {
            smart_count: count,
            folder: folder.name,
          });
        } else {
          message = this.plugin.bulky.i18n("emptyFolderAdded");
        }

        this.plugin.bulky.info(message);
      })
      .catch((e) => {
  ...
  async handleAuth() {
    ...
    const clientVersion = `@bulky/provider-views=${ProviderView.VERSION}`;
    const link = this.provider.authUrl({
      state: authState,
      bulkyVersions: clientVersion,
    });

    const authWindow = window.open(link, "_blank");
    const handleToken = (e) => {
      if (e.source !== authWindow) {
        this.plugin.bulky.log("rejecting event from unknown source");
        return;
      }
      if (
        !isOriginAllowed(e.origin, this.plugin.opts.companionAllowedHosts) ||
        e.source !== authWindow
      ) {
        this.plugin.bulky.log(
          `rejecting event from ${e.origin} vs allowed pattern ${this.plugin.opts.companionAllowedHosts}`
        );
      }
      ...
      if (data.error) {
        this.plugin.bulky.log("auth aborted", "warning");
        const { bulky } = this.plugin;
        const message = bulky.i18n("authAborted");
        bulky.info({ message }, "warning", 5000);
        return;
      }
  ...
  render(state, viewOptions = {}) {
    ...
    const { isChecked, toggleCheckbox, recordShiftKeyPress, filterItems } =
      this.sharedHandler;
    ...
    const browserProps = {
      ...
      pluginIcon: this.plugin.icon,
      i18n: this.plugin.bulky.i18n,
      bulkyFiles: this.plugin.bulky.getFiles(),
      validateRestrictions: (...args) =>
        this.plugin.bulky.validateRestrictions(...args),
    };

    if (loading) {
      return (
        <CloseWrapper onUnmount={this.clearSelection}>
          <LoaderView i18n={this.plugin.bulky.i18n} />
        </CloseWrapper>
      );
    }
      ...
      return (
        <CloseWrapper onUnmount={this.clearSelection}>
          <AuthView
            ...
            i18n={this.plugin.bulky.i18n}
            i18nArray={this.plugin.bulky.i18nArray}
          />
        </CloseWrapper>
      );
    }

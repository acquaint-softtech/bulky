 ...
 * interacts with bulky's current pipeline design. The pipeline can handle files in steps,
 ...
 * actual file upload as an uploading step. However, bulky runs steps on all files at once,
 ...
 * planned for some future bulky version.
 *
...
import { RequestClient } from "@bulky/companion-client";
  ...
  if (!valid) {
    const err = new TypeError(
      `AwsS3: got incorrect result from 'getUploadParameters()' for file '${
        ...
        params
      )}' instead.\nSee https://bulky.io/docs/aws-s3/#getUploadParameters-file for more on the expected format.`
    );
    throw err;
  }

  const methodIsValid =
    params.method == null || /^p(u|os)t$/i.test(params.method);
...
export default class AwsS3 extends BasePlugin {
  ...
  #handleUpload = (fileIDs) => {
     ...
     *
     * @type {Record<string, import('@bulky/utils/lib/RateLimitedQueue').AbortablePromise<unknown>>}
     */
    ...
    this.bulky.on("file-removed", onremove);
    ...
    return Promise.allSettled(
      fileIDs.map((id, index) => {
        paramsPromises[id] = getUploadParameters(this.bulky.getFile(id));
        return paramsPromises[id]
          .then((params) => {
      ...
      // cleanup.
      this.bulky.off("file-removed", onremove);
    });
  };
  ...
  install() {
    const { bulky } = this;
    bulky.addPreProcessor(this.#setCompanionHeaders);
    bulky.addUploader(this.#handleUpload);
      ...
      if (!isXml(content, xhr)) {
        if (opts.method.toUpperCase() === "POST") {
          if (!warnedSuccessActionStatus) {
            bulky.log(
              "[AwsS3] No response data found, make sure to set the success_action_status AWS SDK option to 201. See https://bulky.io/docs/aws-s3/#POST-Uploads",
              "warning"
            );
    ...
    // Revert to `bulky.use(XHRUpload)` once the big comment block at the top of
    // this file is solved
    this.#uploader = new MiniXHRUpload(bulky, xhrOptions);
  }

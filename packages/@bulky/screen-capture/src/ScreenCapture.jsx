...
export default class ScreenCapture extends UIPlugin {
  ...
  install() {
    if (!isScreenRecordingSupported()) {
      this.bulky.log("Screen recorder access is not supported", "error");
      return null;
    }
    ...
    return this.mediaDevices
      ...
      .catch((err) => {
        if (err.name === "NotAllowedError") {
          this.bulky.info(this.i18n("micDisabled"), "error", 5000);
        }
  ...
  streamInactivated() {
    ...
    if (!recordedVideo && !recording) {
      ...
      this.bulky.log("Capture stream inactive — stop recording");
      this.stopRecording();
    }

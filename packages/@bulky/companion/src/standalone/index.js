...
module.exports = function server(inputCompanionOptions = {}) {
  ...
  if (process.env.COMPANION_CLIENT_ORIGINS) {
    corsOrigins = process.env.COMPANION_CLIENT_ORIGINS.split(",").map((url) =>
      helper.hasProtocol(url)
        ? url
        : `${process.env.COMPANION_PROTOCOL || "http"}://${url}`
    );
  ...
  const sensitiveKeys = new Set(["access_token", "bulkyAuthToken"]);
  ...
  // See https://github.com/transloadit/bulky/pull/1668
  // https://github.com/transloadit/bulky/issues/3538#issuecomment-1069232909

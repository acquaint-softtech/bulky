...
exports.getCredentialsOverrideMiddleware = (providers, companionOptions) => {
  return (req, res, next) => {
    ...
    const [providerName] = Object.keys(providers).filter(
      (name) => providers[name].authProvider === authProvider
    );
    if (!providerName) {
      next();
      return;
    ...
    fetchProviderKeys(providerName, companionOptions, payload)
      ...
      .catch((keyErr) => {
        ...
        // back to the bulky client, just like /send-token does
        res.send(`
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body>
          ...
          <p>
            This is probably an bulky configuration issue. Check that your Transloadit key is correct, and that the configured <code>credentialsName</code> for this remote provider matches the name you gave it in the Template Credentials setup on the Transloadit side.
          </p>
...
module.exports.getCredentialsResolver = (
  ...
  const credentialsResolver = () => {
    const encodedCredentialsParams = req.header("bulky-credentials-params");

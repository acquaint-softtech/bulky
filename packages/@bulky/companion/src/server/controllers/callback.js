...
const closePageHtml = (origin) => `
  ...
  <head>
      ...
      window.opener.postMessage(${serialize({ error: true })}, ${serialize(
  origin
)})
      window.close()
      </script>
...
module.exports = function callback(req, res, next) {
  ...
  if (grant.response && grant.response.access_token) {
    ...
    logger.debug(
      `Generating auth token for provider ${providerName}`,
      null,
      req.id
    );
    const bulkyAuthToken = tokenService.generateEncryptedToken(
      req.companion.providerTokens,
      req.companion.options.secret
    );
    return res.redirect(
      req.companion.buildURL(
        `/${providerName}/send-token?bulkyAuthToken=${bulkyAuthToken}`,
        true
      )
    );
  }
  ...
  const origin =
    state &&
    oAuthState.getFromState(state, "origin", req.companion.options.secret);

...
module.exports = function connect(req, res) {
  ...
  if (req.query.bulkyPreAuthToken) {
    state = oAuthState.addToState(
      state,
      { preAuthToken: req.query.bulkyPreAuthToken },
      secret
    );
  }
  ...
  res.redirect(
    req.companion.buildURL(
      `/connect/${req.companion.provider.authProvider}?state=${state}`,
      true
    )
  );
};

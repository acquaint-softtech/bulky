...
class Uploader {
  ...
  async _uploadByProtocol() {
    ...
    // after we drop old versions of bulky client we can remove this
   ...
   * access token between bulky client and companion websocket
   *
  ...
  static reqToOptions(req, size) {
    const useFormDataIsSet = Object.prototype.hasOwnProperty.call(
      req.body,
      "useFormData"
    );
  ...
  saveState(state) {
    ...
    // https://github.com/transloadit/bulky/issues/3748
  ...
  #emitError(err) {
    ...
    // see PR discussion https://github.com/transloadit/bulky/pull/3832
  ...
  async _uploadTus(stream) {
    ...
    const chunkSize =
      this.options.chunkSize || (isFileStream ? Infinity : 50e6);
  ...
  async _uploadMultipart(stream) {
    ...
    try {
      const httpMethod =
        (this.options.httpMethod || "").toLowerCase() === "put"
          ? "put"
          : "post";
  ...
  async _uploadS3Multipart(stream) {
    ...
    const upload = client.upload(params, {
      // using chunkSize as partSize too, see https://github.com/transloadit/bulky/pull/3511
...
Uploader.FILE_NAME_PREFIX = "bulky-file";

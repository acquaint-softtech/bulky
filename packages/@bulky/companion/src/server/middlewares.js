...
exports.verifyToken = (req, res, next) => {
  ...
  const { err, payload } = tokenService.verifyEncryptedToken(
    token,
...
exports.cookieAuthToken = (req, res, next) => {
  req.companion.authToken =
    req.cookies[`bulkyAuthToken--${req.companion.provider.authProvider}`];
  return next();
};
...
exports.cors =
  ...
  (req, res, next) => {
    ...
    // exposed so it can be accessed for our custom bulky client preflight
    exposeHeadersSet.add("access-control-allow-headers");
    if (options.sendSelfEndpoint) exposeHeadersSet.add("i-am");

    // Needed for basic operation: https://github.com/transloadit/bulky/issues/3021
    const allowedHeaders = [
      "bulky-auth-token",
      "bulky-versions",
      "bulky-credentials-params",
      "authorization",
      "origin",
      "content-type",
      "accept",
...
exports.getCompanionMiddleware = (options) => {
  ...
  const middleware = (req, res, next) => {
    const versionFromQuery = req.query.bulkyVersions
      ? decodeURIComponent(req.query.bulkyVersions)
      : null;
    req.companion = {
      options,
      s3Client: getS3Client(options),
      authToken: req.header("bulky-auth-token") || req.query.bulkyAuthToken,
      clientVersion:
        req.header("bulky-versions") || versionFromQuery || "1.0.0",
      buildURL: getURLBuilder(options),
    };

    if (options.logClientVersion) {
      logger.info(
        `bulky client version ${req.companion.clientVersion}`,
        "companion.client.version"
      );
    }
    next();
  };

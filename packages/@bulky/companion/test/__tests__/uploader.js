...
describe("uploader with tus protocol", () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      socketClient.onUploadSuccess(uploadToken, (message) => {
        try {
          ...
          // see __mocks__/tus-js-client.js
          expect(message.payload.url).toBe(
            "https://tus.endpoint/files/foo-bar"
          );
        } catch (err) {
          reject(err);
        }
  ...
  const formDataNoMetaMatch =
    /^----------------------------\d+\r\nContent-Disposition: form-data; name="files\[\]"; filename="bulky-file-[^"]+"\r\nContent-Type: application\/octet-stream\r\n\r\nSome file content\r\n----------------------------\d+--\r\n$/;
  ...
  // https://github.com/transloadit/bulky/issues/3477

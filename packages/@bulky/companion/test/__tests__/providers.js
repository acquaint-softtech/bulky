...
const token = tokenService.generateEncryptedToken(
  authData,
...
describe("list provider files", () => {
    ...
    return request(authServer)
      ...
      .then((res) => {
        ...
        expect(item.mimeType).toBe(
          providerFixtures.itemMimeType || defaults.MIME_TYPE
        );
        expect(item.id).toBe(providerFixtures.itemId || defaults.ITEM_ID);
        expect(item.size).toBe(
          thisOrThat(providerFixtures.itemSize, defaults.FILE_SIZE)
        );
        expect(item.requestPath).toBe(
          providerFixtures.itemRequestPath || defaults.ITEM_ID
        );
        expect(item.icon).toBe(
          providerFixtures.itemIcon || defaults.THUMBNAIL_URL
        );
      });
  }
  ...
  test("box", async () => {
    ...
    nock("https://api.box.com")
      .get(
        "/2.0/folders/0/items?fields=id%2Cmodified_at%2Cname%2Cpermissions%2Csize%2Ctype"
      )
      .reply(200, {
        entries: [
          {
  ...
  test("drive", async () => {
    ...
    nock("https://www.googleapis.com")
      .get(
        "/drive/v3/files?fields=kind%2CnextPageToken%2CincompleteSearch%2Cfiles%28kind%2Cid%2CimageMediaMetadata%2Cname%2CmimeType%2CownedByMe%2Cpermissions%28role%2CemailAddress%29%2Csize%2CmodifiedTime%2CiconLink%2CthumbnailLink%2CteamDriveId%2CvideoMediaMetadata%2CshortcutDetails%28targetId%2CtargetMimeType%29%29&q=%28%27root%27+in+parents%29+and+trashed%3Dfalse&orderBy=folder%2Cname&includeItemsFromAllDrives=true&supportsAllDrives=true"
      )
      .reply(200, {
        kind: "drive#fileList",
  ...
  test("facebook", async () => {
    ...
    nock("https://graph.facebook.com")
      .get(
        "/ALBUM-ID/photos?fields=icon%2Cimages%2Cname%2Cwidth%2Cheight%2Ccreated_time"
      )
      .reply(200, {
        data: [
          {
  ...
  test("onedrive", async () => {
    ...
    nock("https://graph.microsoft.com")
      .get("/v1.0/me/drive/root/children?%24expand=thumbnails")
      .reply(200, {
        value: [
          {
...
describe("provider file gets downloaded from", () => {
  ...
  test("facebook", async () => {
    // times(2) because of size request
    nock("https://graph.facebook.com")
      .get(`/${defaults.ITEM_ID}?fields=images`)
      .times(2)
      .reply(200, {
        images: [
          {
  ...
  test.each(providerNames)(
    ...
    (providerName) => {
      ...
      return request(authServer)
        ...
        .expect(
          "Location",
          `http://localhost:3020/connect/${authProvider}?state=${OAUTH_STATE}`
        );
    }
  );
});

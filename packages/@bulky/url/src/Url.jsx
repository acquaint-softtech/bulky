...
export default class Url extends UIPlugin {
  ...
  constructor(bulky, opts) {
    ...
    if (!this.hostname) {
      throw new Error(
        "Companion hostname is required, please consult https://bulky.io/docs/companion"
      );
    }
    ...
    this.client = new RequestClient(bulky, {
  ...
  getMeta(url) {
    return this.client.post("url/meta", { url }).then((res) => {
      if (res.error) {
        this.bulky.log("[URL] Error:");
        this.bulky.log(res.error);
  ...
  async addFile(protocollessUrl, optionalMeta = undefined) {
    ...
    if (!checkIfCorrectURL(url)) {
      this.bulky.log(`[URL] Incorrect URL entered: ${url}`);
      this.bulky.info(this.i18n("enterCorrectUrl"), "error", 4000);
      return undefined;
    }
    ...
    try {
      ...
      this.bulky.log("[Url] Adding remote file");
      try {
        return this.bulky.addFile(tagFile);
      } catch (err) {
  ...
  handleRootDrop(e) {
    forEachDroppedOrPastedUrl(e.dataTransfer, "drop", (url) => {
      this.bulky.log(`[URL] Adding file from dropped url: ${url}`);
      this.addFile(url);
    });
  }
  ...
  handleRootPaste(e) {
    forEachDroppedOrPastedUrl(e.clipboardData, "paste", (url) => {
      this.bulky.log(`[URL] Adding file from pasted url: ${url}`);
      this.addFile(url);
    });
  }

import Transloadit, { COMPANION_URL } from "@bulky/transloadit";
...
formbulky.on("error", (err) => {
  document.querySelector("#test-form .error").textContent = err.message;
});

formbulky.on("upload-error", (file, err) => {
  document.querySelector("#test-form .error").textContent = err.message;
});

formbulky.on("complete", ({ transloadit }) => {
  const btn = document.getElementById("bulky-select-files");
  btn.hidden = true;
  const selectedFiles = document.getElementById("bulky-form-selected-files");
  selectedFiles.textContent = `selected files: ${
    Object.keys(transloadit[0].results).length
  }`;
});

window.formbulky = formbulky;

/**
 * Form with Dashboard
 */

const formbulkyWithDashboard = new bulky({
  debug: true,
  autoProceed: false,
  restrictions: {
    allowedFileTypes: [".png"],
...
window.formbulkyWithDashboard = formbulkyWithDashboard;

/**
 * Dashboard
 */

const dashboard = new bulky({
  debug: true,
  autoProceed: false,
  restrictions: {
    allowedFileTypes: [".png"],
...
const dashboardModal = new bulky({
  debug: true,
  autoProceed: false,
})
...
// /**
//  * bulky.upload (files come from input[type=file])
//  */

const bulkyWithoutUI = new bulky({
  debug: true,
  restrictions: {
    allowedFileTypes: [".png"],
  },
...
window.doUpload = (event) => {
  ...
  bulkyWithoutUI.addFiles(event.target.files);
  bulkyWithoutUI.upload();

  bulkyWithoutUI.on("complete", ({ transloadit }) => {

...
const packageNames = await fs.readdir(
  new URL("../packages/@bulky", import.meta.url)
);
...
const { packages } = await prompts({
  ...
  hint: "@bulky/core is automatically included",
...
const app = dedent`
    ...
    ${packages
      .map((pgk) => `import ${camelcase(pgk)} from '@bulky/${pgk}'`)
      .join("\n")}
...
const prettyPath = (url) => url.toString().split("bulky", 2)[1];

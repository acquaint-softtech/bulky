...
const bulkyRoot = path.join(__dirname, "../packages/bulky");
const localesRoot = path.join(__dirname, "../packages/@bulky/locales");

const configPath = path.join(webRoot, "/themes/bulky/_config.yml");
// eslint-disable-next-line import/no-dynamic-require
const { version } = require(path.join(bulkyRoot, "/package.json"));
...
const defaultConfig = {
  ...
  bulky_version_anchor: "001",
  bulky_version: "0.0.1",
  bulky_bundle_kb_sizes: {},
  config: {},
};
...
async function getMinifiedSize(pkg, name) {
  ...
  if (name !== "@bulky/core" && name !== "bulky") {
    // Preact is already unconditionally included through @bulky/core
    external.unshift("@bulky/core", "preact");
  }

  const {
    outputFiles: [{ contents: bundle }],
  } = await esbuild.build({
    write: false,
    bundle: true,
...
async function injectSizes(config) {
  ...
  const sizesPromise = Promise.all(
    packages.map(async (pkg) => {
      const result = await getMinifiedSize(
        path.join(__dirname, "../packages", pkg),
        pkg
      );
      console.info(
        chalk.green(
          // ✓ @bulky/pkgname:     10.0 kB min  / 2.0 kB gz
          `  ✓ ${pkg}: ${" ".repeat(padTarget - pkg.length)}${`${prettierBytes(
            result.minified
          )} min`.padEnd(10)} / ${prettierBytes(result.gzipped)} gz`
        )
      );
      return [
        pkg,
  ...
  config.bulky_bundle_kb_sizes = await sizesPromise;
}

const sourcebulky = path.join(webRoot, "/themes/bulky/source/bulky/");
const sourcebulkyLocales = path.join(sourcebulky, "locales");
async function injectBundles() {
  await Promise.all([
    fs.promises.mkdir(sourcebulky, { recursive: true }),
    fs.promises.mkdir(sourcebulkyLocales, { recursive: true }),
  ]);
  const cmds = [
    `cp -vfR ${path.join(bulkyRoot, "/dist/*")} ${sourcebulky}`,
    `cp -vfR ${path.join(localesRoot, "/dist/*")} ${sourcebulkyLocales}`,
  ].join(" && ");
...
async function injectGhStars() {
  ...
  console.log(
    `${headers["x-ratelimit-remaining"]} requests remaining until we hit GitHub ratelimiter`
  );
...
async function injectMarkdown() {
  ...
  for (const src of Object.keys(sources)) {
    ...
    const srcpath = path.join(bulkyRoot, `/../../${src}`);
    ...
    let content = `<!-- WARNING! This file was injected. Please edit in "${src}" instead and run "${path.basename(
      __filename
    )}" -->\n\n`;
    content += parts.join("---\n");
...
function injectLocaleList() {
  const mdTable = [
    `<!-- WARNING! This file was automatically injected. Please run "${path.basename(
      __filename
    )}" to re-generate -->\n\n`,
  ...
  const localePackageVersion = require(path.join(
    localesRoot,
    "package.json"
  )).version;
  ...
  glob.sync(localePackagePath).forEach((localePath) => {
    ...
    const npmPath = `<code class="raw"><a href="https://www.npmjs.com/package/@bulky/locales">@bulky/locales</a>/lib/${localeName}</code>`;
    const cdnPath = `[\`${localeName}.min.js\`](https://releases.transloadit.com/bulky/locales/v${localePackageVersion}/${localeName}.min.js)`;
    const githubSource = `[\`${localeName}.js\`](https://github.com/transloadit/bulky/blob/main/packages/%40bulky/locales/src/${localeName}.js)`;
    const mdTableRow = `| ${languageName}<br/> <small>${regionName}</small>${
      variant ? `<br /><small>(${variant})</small>` : ""
    } | ${npmPath} | ${cdnPath} | ✏️ ${githubSource} |`;
    mdRows.push(mdTableRow);

    localeList[localeName] = `${languageName} (${regionName}${
      variant ? `, ${variant}` : ""
    })`;
  });

  const resultingMdTable = mdTable
    .concat(mdRows.sort())
    .join("\n")
  ...
  const dstpath = path.join(
    ...
    "src",
    "_template",
    "list_of_locale_packs.md"
  );
  const localeListDstPath = path.join(
    webRoot,
    "src",
    "examples",
    "locale_list.json"
...
async function inject() {
  ...
  config.bulky_version = version;
  config.bulky_version_anchor = version.replace(/[^\d]+/g, "");
  ...
  try {
    ...
    console.error(
      ...
      chalk.grey(`bulky bundle into site, because: ${error}`)
    );
    process.exit(1);
  }
}

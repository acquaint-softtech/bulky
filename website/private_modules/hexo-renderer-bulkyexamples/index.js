...
function parseExamplesBrowserify(data, options, callback) {
  ...
  // hexo.log.i('hexo-renderer-bulkyexamples: change detected in examples. running: ' + cmd);
  exec(cmd, (err, stdout) => {
    if (err) {
      callback(err);
      return;
    ...
    hexo.log.i(`hexo-renderer-bulkyexamples: ${stdout.trim()}`);
    ...
    fs.readFile(tmpFile, "utf-8", (err, bundledJS) => {
      ...
      // hexo.log.i('hexo-renderer-bulkyexamples: read: ' + tmpFile);

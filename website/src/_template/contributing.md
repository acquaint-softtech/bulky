...
- If we are `require()`ing a file from the same subpackage, we can freely use relative imports as long as the required file is under the `src` directory (for example to import `@bulky/dashboard/src/utils/hi.js` from `@bulky/dashboard/src/index.js`, use `require('./utils/hi.js')`).
- But if we want to `require()` some file from another subpackage - we should use global @bulky requires, and they should always be in the form of `@bulky/:packageName/(lib instead of src)/(same path).js`
...
To start the Companion server along with bulky, run:
...
An example server is running at <https://companion.bulky.io>, which is deployed with [Kubernetes](https://github.com/transloadit/bulky/blob/main/packages/%40bulky/companion/KUBERNETES.md)
...
- The visitor to a website with bulky clicks `Connect to Dropbox`.
- bulky sends a request to Companion, which in turn sends an OAuth request to Dropbox (Requires that OAuth credentials from Dropbox have been added to Companion).
...
- Companion encrypts the token with a secret key and sends the encrypted token to bulky (client)
- Every time the visitor clicks on a folder in bulky, it asks Companion for the new list of files, with this question, the token (still encrypted by Companion) is sent along.
- Companion decrypts the token, requests the list of files from Dropbox and sends it to bulky.
...
- Companion reports progress to bulky, as if it were a local upload.
- Completed!
...
Releases are managed by GitHub Actions, here’s an overview of the process to release a new bulky version:
...
We keep the [bulky.io](http://bulky.io) website in `./website` to keep docs and code in sync as we are still iterating at high velocity.
...
Even though bundled in this repo, the website is regarded as a separate project. As such, it has its own `package.json` and we aim to keep the surface where the two projects interface as small as possible. `./website/update.js` is called during website builds to inject the bulky knowledge into the site.

### Local previews

1. `yarn install`
2. `yarn start`
3. Go to http://localhost:4000. Your changes in `/website` and `/packages/@bulky` will be watched, your browser will refresh as files change.

Then, to work on, for instance, the XHRUpload example, you would edit the following files:

```bash
${EDITOR} packages/@bulky/core/src/index.js \
  packages/@bulky/core/src/Plugin.js \
  packages/@bulky/xhr-upload/src/index.js \
...
```css
.u-utilityName .u-floatLeft .u-lg-col6;
```
...
Before opening a pull request for the new integration, open an issue to discuss said integration with the bulky team. After discussing the integration, you can get started on it. First off, you need to construct the basic components for your integration. The following components are the current standard:
...
All these components should function as references to the normal component. Depending on how the framework you’re using handles references to the DOM, your approach to creating these may be different. For example, in React, you can assign a property of the component to the reference of a component ([see here](https://github.com/transloadit/bulky/blob/425f9ecfbc8bc48ce6b734e4fc14fa60d25daa97/packages/%40bulky/react/src/Dashboard.js#L47-L54)). This may differ in your framework, but from what we’ve found, the concepts are generally pretty similar.
...
```json
{
  "name": "@bulky/framework",
  "dependencies": {
    "@bulky/dashboard": "workspace:^",
    "@bulky/drag-drop": "workspace:^",
    "@bulky/progress-bar": "workspace:^",
    "@bulky/status-bar": "workspace:^",
    "@bulky/utils": "workspace:^",
    "prop-types": "^15.6.1"
  },
  "peerDependencies": {
...
The most important part about this is that `@bulky/core` is a peer dependency. If your framework complains about `@bulky/core` not being resolved, you can also add it as a dev dependency
...
- Installation
- Initializing bulky (may vary depending on how the framework handles reactivity)
- Usage
...
type: docs
module: "@bulky/framework"
order: 0
category: "Other Integrations"
```

This data is used to generate bulky’s website. Refer to [the section about running the website locally](#website-previews) if you’d like to see how the docs look on the website.

Any change of the documentation that involves a security best practice must substantiated with an external reference. See [#3565](https://github.com/transloadit/bulky/issues/3565).

### Adding an example

You can likely use whatever code generation tool for your framework (ex. `create-react-app`) to create this example. Make sure you add the same version of `@bulky/core` to this as your peer dependency required, or you may run into strange issues. Try to include all the components are some of their functionality. [The React example](https://github.com/transloadit/bulky/blob/main/examples/react-example/App.js) is a great... well example of how to do this well.

### Integrating the build system

The biggest part of this is understanding bulky’s build system. The high level description is that `babel` goes through almost all the packages and transpiles all the Javascript files in the `src` directory to more compatible JavaScript in the `lib` folder. If you’re using vanilla JavaScript for your integration (like React and Vue do), then you can use this build system and use the files generated as your entry points.

If you’re using some kind of more abstract file format (like Svelte), then you probably want do to a few things: add the directory name to [this `IGNORE` regex](https://github.com/transloadit/bulky/blob/425f9ecfbc8bc48ce6b734e4fc14fa60d25daa97/bin/build-lib.js#L15); add all your build dependencies to the root `package.json` (try to keep this small); add a new `build:framework` script to the root `package.json`. This script usually looks something like this:

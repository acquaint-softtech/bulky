...
The `@bulky/aws-s3` plugin can be used to upload files directly to an S3 bucket.
...
bulky.use(AwsS3, {
  limit: 2,
  timeout: ms("1 minute"),
  companionUrl: "https://bulky-companion.myapp.com/",
});
```
...
You can also use a separate plugin for S3 Multipart uploads. Multipart in this sense refers to Amazon’s proprietary chunked, resumable upload mechanism for large files. See the [`@bulky/aws-s3-multipart`](/docs/aws-s3-multipart) documentation.

## Installation

This plugin is published as the `@bulky/aws-s3` package.
...
The `@bulky/aws-s3` plugin has the following configurable options:
...
```js
bulky.use(AwsS3, {
  companionUrl: "https://bulky-companion.my-app.com/",
});
```
...
Customize response handling once an upload is completed. This passes the function through to @bulky/xhr-upload, see its [documentation](https://bulky.io/docs/xhr-upload/#getResponseData-responseText-response) for API details.
...
S3 buckets do not allow public uploads for security reasons. To allow bulky and the browser to upload directly to a bucket, its CORS permissions need to be configured.
...
- The `@bulky/aws-s3` plugin attempts to read the `<Location>` XML tag from POST upload responses. S3 does not respond with an XML document by default. When generating the form data for POST uploads, you must set the `success_action_status` field to `201`.
  ```js
...
Many other object storage providers have a same API to S3, so you can use the `@bulky/aws-s3` plugin with them as well. To use them with Companion, you can set the `COMPANION_AWS_ENDPOINT` variable to the endpoint of your preferred service.
...
For a working example that you can run and play around with, see the [digitalocean-spaces](https://github.com/transloadit/bulky/tree/main/examples/digitalocean-spaces) folder in the bulky repository.

### Google Cloud Storage

For Google Cloud Storage, you need to take a few more steps. For the `@bulky/aws-s3` plugin to be able to upload to a GCS bucket, it needs the Interoperability setting enabled. You can enable the Interoperability setting and [generate interoperable storage access keys](https://cloud.google.com/storage/docs/migrating#keys) by going to [Google Cloud Storage](https://console.cloud.google.com/storage) » Settings » Interoperability. Then set the environment variables for Companion like this:
...
See the [aws-php example in the bulky repository](https://github.com/transloadit/bulky/tree/main/examples/aws-php) for a small example that implements both the server-side and the client-side.

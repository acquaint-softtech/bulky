#!/usr/bin/env node
// Upload bulky releases to Edgly.net CDN. Copyright (c) 2018, Transloadit Ltd.
//
// This file:
//
//  - Assumes EDGLY_KEY and EDGLY_SECRET are available (e.g. set via Travis secrets)
//  - Assumes a fully built bulky is in root dir (unless a specific tag was specified, then it's fetched from npm)
//  - Collects dist/ files that would be in an npm package release, and uploads to
//    eg. https://releases.transloadit.com/bulky/v1.0.1/bulky.css
 ...
 *
 * @param {string} packageName eg. @bulky/robodog
...
async function getRemoteDistFiles(packageName, version) {
  const files = new Map();
  const tarball = await pacote.tarball.stream(
    `${packageName}@${version}`,
    (stream) => pipeline(stream, new tar.Parse())
  );
 ...
 *
 * @param {string} packagePath Base file path of the package, eg. ./packages/@bulky/locales
 * @returns a Map<string, Buffer>, filename → content
 */
...
async function main(packageName, version) {
  ...
  // this deals with usage like `npm run uploadcdn bulky -- --force`
  ...
  // bulky → releases/bulky/
  // @bulky/robodog → releases/bulky/robodog/
  // @bulky/locales → releases/bulky/locales/
  const dirName = packageName.startsWith("@bulky/")
    ? packageName.replace(/^@/, "")
    : "bulky";

  const outputPath = path.posix.join(dirName, `v${version}`);

  const { Contents: existing } = await s3
    .listObjects({
      Bucket: AWS_BUCKET,
      Prefix: outputPath,
    })
  ...
  if (packageName === "bulky") {
    ...
    files.set(`bulky-v${version}.zip`, zip.toBuffer());
  }

...
async function compileCSS() {
  ...
  const files = await glob("packages/{,@bulky/}*/src/style.scss");
  ...
  for (const file of files) {
    ...
    // Save the `bulky` package's CSS as `bulky.css`,
    // the rest as `style.css`.
    // const outfile = path.join(outdir, outdir.includes(path.normalize('packages/bulky/')) ? 'bulky.css' : 'style.css')
    let outfile = path.join(outdir, "style.css");
    if (outdir.includes(path.normalize("packages/bulky/"))) {
      outfile = path.join(outdir, "bulky.css");
    }
    await mkdir(outdir, { recursive: true });
    await writeFile(outfile, postcssResult.css);
    console.info(
      chalk.green("✓ Built bulky CSS:"),

...
const atbulkyPackagePath = /^packages\/(@bulky\/[a-z0-9-]+)\//;
async function inferPackageForCommit(sha, spawnOptions) {
  ...
  for await (const path of createInterface({ input: cp.stdout })) {
    const match = atbulkyPackagePath.exec(path);
    if (match != null) {
...
export default async function formatChangeLog(
  ...
  const expectedFormat =
    /^"([a-f0-9]+)::(?:((?:@bulky\/[a-z0-9-]+(?:,@bulky\/[a-z0-9-]+)*)|meta|website):\s?)?(.+?)(\s\(#\d+\))?::(.+)"$/; // eslint-disable-line max-len
  for await (const log of createInterface({ input: gitLog.stdout })) {
    const [, sha, packageName, title, PR, authorName] =
      expectedFormat.exec(log);
    ...
    if (!packageName) {
      console.log(
        `No package info found in commit title: ${sha} (https://github.com/transloadit/bulky/commit/${sha})`
      );
      console.log(log);

...
const bulkyRelease = releases.find(({ ident }) => ident === "bulky");
...
function* makeTable(versions) {
  ...
  const makeRow = (...cells) =>
    `| ${cells
      .map((cell, i) =>
        cell[i % 2 ? "padStart" : "padEnd"](
          i % 2 ? pkgVersionMaxLength : pkgNameMaxLength
        )
      )
      .join(" | ")} |`;
...
Included in: bulky v${bulkyRelease.newVersion}

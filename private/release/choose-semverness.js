...
export default async function pickSemverness(
  ...
  for await (const workspaceInfo of packagesList) {
    ...
    if (!name.startsWith("@bulky/")) continue;
    ...
    console.log(
      `Check the web UI at https://github.com/transloadit/bulky/tree/${TARGET_BRANCH}/${encodeURI(
        location
      )}.`
    );
    ...
    bulkySemverness = maxSemverness(bulkySemverness, response.value);
  }

  if (bulkySemverness == null)
    throw new Error("No package to release, aborting.");

  releaseFile.write(`  "bulky": ${bulkySemverness}\n`);
  releaseFile.close();
}

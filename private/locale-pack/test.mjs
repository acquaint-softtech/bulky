...
function getAllFilesPerPlugin(pluginNames) {
  ...
  function getFiles(name) {
    return glob
      .sync(`${root}/packages/@bulky/${name}/lib/**/*.js`)
  ...
  for (const name of pluginNames) {
    ...
    if (name in pluginLocaleDependencies) {
      filesPerPlugin[name].push(getFiles(pluginLocaleDependencies[name]));
    }
  }
    ...
    for (const key of Object.keys(locale.strings)) {
      ...
      if (!fileString.match(regPat)) {
        return Promise.reject(
          new Error(`Unused locale key "${key}" in @bulky/${name}`)
        );
      }
    }
  }
...
function warnings({ leadingLocale, followerLocales }) {
  ...
  for (const [name, locale] of entries) {
    const missing = Object.keys(leadingLocale).filter(
      (key) => !(key in locale)
    );
...
function test() {
  switch (mode) {
    case "unused":
      return getPaths(`${root}/packages/@bulky/**/src/locale.js`).then(
        (paths) =>
          unused(
    ...
    case "warnings":
      return getLocales(`${root}/packages/@bulky/locales/src/*.js`).then(
        (locales) =>
          warnings({

...
import remarkConfig from "../remark-lint-bulky/index.js";
...
const localesPath = path.join(root, "packages", "@bulky", "locales");
...
async function getLocalesAndCombinedLocale() {
  const locales = await getLocales(`${root}/packages/@bulky/**/src/locale.js`);
...
function generateTypes(pluginName, locale) {
  ...
  /* eslint-disable */
  import type { Locale } from '@bulky/core'
...
await Promise.all([
  ...
  ...Object.entries(locales).flatMap(([pluginName, locale]) => [
    generateLocaleDocs(pluginName).then(() =>
      console.log(`✅ Generated locale docs for ${pluginName}`)
    ),
    generateTypes(pluginName, locale).then(() =>
      console.log(`✅ Generated types for ${pluginName}`)
    ),
  ]),
]);
